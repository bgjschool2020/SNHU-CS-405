...
bool run_query(sqlite3 *db, const std::string &sql, std::vector<user_record> &records)
{
    ...
    // Regular expression to detect "or value=value;" pattern
    std::regex injection_pattern(R"(\sor\s+['"]?\w+['"]?\s*=\s*['"]?\w+['"]?\s*;?)");

    // https://en.cppreference.com/w/cpp/regex/regex_search
    if (std::regex_search(lower_sql, injection_pattern))
    {
        std::cout << "Potential SQL Injection detected. Stopped: " << sql << std::endl;
        return false;
    }
